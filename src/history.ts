...
export class Time2BlocksHistoryLoader {
  ...
  private readonly mempoolApi = 'https://mempool.space/testnet/api/';
...
export class Time2BlockMempoolConn extends Time2BlockConnection {
  ...
  connect(): void {
    ...
    this.connection = websocket.initServer({
      options: ["blocks"]
    });
...
export class Time2BlocksBlockchainInfoConn extends Time2BlockConnection {
  private readonly link = 'wss://ws.blockchain.info/inv';
  ...
  private onConnect(conn: any): void {
    this.conn = conn;
    ...
    this.command({ op: "blocks_sub" });
  }

  private onMessage(packet: any): void {
    if (packet.type != 'utf8')
      return;
  ...
  private command(msg: object): void {
